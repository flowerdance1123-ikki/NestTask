...
enum ProgressService {
    static func toggleCompletion(
        for step: ExecutionStep,
        in task: ExecutionTask,
        descendantSteps: [ExecutionStep],
        at date: Date = Date()
    ) {
        ...
        syncAncestorCompletion(from: step, in: task, at: date)
        refreshCompletionState(for: task, reopensCompletedTask: true)
    }
    ...
    private static func syncAncestorCompletion(from changedStep: ExecutionStep, in task: ExecutionTask, at date: Date) {
        guard
        ...
        syncAncestorCompletion(from: parentStep, in: task, at: date)
    }
}
