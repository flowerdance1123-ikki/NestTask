...
enum NotificationSchedulingService {
    ...
    static func refreshScheduledNotifications(
        ...
        let activeScheduledTasks = executionTasks.filter { task in
        ...
        let activeTaskTemplateIDs = Set(activeScheduledTasks.compactMap(\.templateID))
        let templateSnapshots = templates.compactMap { template -> TemplateNotificationSnapshot? in
            guard
                ...
                !activeTaskTemplateIDs.contains(template.id)
            else {
                return nil
            }

            return nextTemplateNotificationSnapshot(
                for: template,
...
enum ProgressService {
    ...
    static func refreshCompletionState(for task: ExecutionTask) {
