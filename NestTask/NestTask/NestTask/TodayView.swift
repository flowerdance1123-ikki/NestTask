...
enum ProFeatureLimits {
    ...
    static let freeMaximumDepth = 1
}

private enum TaskHierarchyLabels {
    static func name(forDepth depth: Int) -> String {
        switch depth {
        case 0:
            return "親タスク"
        case 1:
        ...
        case 2:
            return "孫タスク"
        default:
            return "下位タスク Lv.\(depth + 1)"
        }
    }

    static func nameForAddingUnder(parentDepth depth: Int) -> String {
        name(forDepth: depth + 1)
    }
}

private struct DraftTemplateStepNode: Identifiable {
    let step: DraftTemplateStep
    let depth: Int
    let children: [DraftTemplateStepNode]
...
private enum DraftTemplateHierarchySupport {
    static func nodes(from steps: [DraftTemplateStep]) -> [DraftTemplateStepNode] {
        ...
        func build(parentID: UUID?, depth: Int) -> [DraftTemplateStepNode] {
    ...
    static func flattenedSteps(from steps: [DraftTemplateStep]) -> [DraftTemplateStep] {
        func flatten(_ nodes: [DraftTemplateStepNode]) -> [DraftTemplateStep] {
            nodes.flatMap { node in
                [node.step] + flatten(node.children)
            }
        }

        return flatten(nodes(from: steps))
    }

    static func depth(of stepID: UUID, in steps: [DraftTemplateStep]) -> Int {
        let stepsByID = Dictionary(uniqueKeysWithValues: steps.map { ($0.id, $0) })
        var depth = 0
        var visitedIDs = Set<UUID>()
        var currentParentID = stepsByID[stepID]?.parentID

        while let parentID = currentParentID, let parent = stepsByID[parentID], !visitedIDs.contains(parentID) {
    ...
    static func descendantIDs(of stepID: UUID, in steps: [DraftTemplateStep]) -> Set<UUID> {
        var descendants = Set<UUID>()
        var pending = [stepID]

        while let parentID = pending.popLast() {
            let children = steps.filter { $0.parentID == parentID }
...
private enum AppInfoProUpgradePresentation: Identifiable {
    case general
    case reason(ProUpgradeReason)
...
enum TemplateHierarchyBuilder {
    static func nodes(from steps: [TemplateStep]) -> [TemplateStepNode] {
        let stepIDs = Set(steps.map(\.id))
        let groupedSteps = Dictionary(grouping: steps) { step -> UUID? in
            if let parentID = step.parentID, stepIDs.contains(parentID), parentID != step.id {
                return parentID
            }
            return nil
        }

        func build(parentID: UUID?, depth: Int, visitedIDs: Set<UUID>) -> [TemplateStepNode] {
            groupedSteps[parentID, default: []]
                .sorted { lhs, rhs in
                    if lhs.sortIndex == rhs.sortIndex {
                        return lhs.title < rhs.title
                ...
                .map { step in
                    let nextVisitedIDs = visitedIDs.union([step.id])
                    return TemplateStepNode(
                        step: step,
                        depth: depth,
                        children: build(parentID: step.id, depth: depth + 1, visitedIDs: nextVisitedIDs)
                    )
                }
        }

        return build(parentID: nil, depth: 0, visitedIDs: [])
    }
}
...
enum ExecutionHierarchyBuilder {
    static func nodes(from steps: [ExecutionStep]) -> [ExecutionStepNode] {
        ...
        func build(parentID: UUID?, depth: Int, visitedIDs: Set<UUID>) -> [ExecutionStepNode] {
            groupedSteps[parentID, default: []]
                .sorted { lhs, rhs in
                    if lhs.sortIndex == rhs.sortIndex {
                        return lhs.title < rhs.title
...
private enum ExecutionStepHierarchySupport {
    static func depth(of stepID: UUID, in steps: [ExecutionStep]) -> Int {
        let stepsByID = Dictionary(uniqueKeysWithValues: steps.map { ($0.id, $0) })
        var depth = 0
        var visitedIDs = Set<UUID>()
        var currentParentID = stepsByID[stepID]?.parentID
...
struct MainTabView: View {
    ...
    private func scheduleInitialPreparation() {
        guard !didRunInitialPreparation, preparationTask == nil else { return }

        didRunInitialPreparation = true
        preparationTask = Task {
            await prepareScheduledTasks(requestNotificationAuthorization: false, delayNanoseconds: 1_500_000_000)
        }
    }
...
struct ScheduleView: View {
    ...
    private func isGeneratedTask(_ task: ExecutionTask, on date: Date) -> Bool {
        switch task.source {
        case .scheduled:
            return calendar.isDate(task.scheduledDate, inSameDayAs: date)
        case .manual:
            return task.templateID != nil
                && (calendar.isDate(task.startDate, inSameDayAs: date)
...
struct TemplatesView: View {
    ...
    @State private var proUpgradeReason: ProUpgradeReason?
    ...
    var body: some View {
        ScreenContainer {
            ScreenHeader(
                ...
                actions: [
                    HeaderAction(systemName: "square.and.arrow.up", tint: NestTaskStyle.ink, label: "書き出し", isLocked: !purchaseManager.isPro) {
                        exportTemplates()
                    },
                    HeaderAction(systemName: "square.and.arrow.down", tint: NestTaskStyle.ink, label: "読み込み", isLocked: !purchaseManager.isPro) {
                        importTemplates()
                    },
                    HeaderAction(systemName: "plus", tint: NestTaskStyle.teal, label: "追加", isLocked: !purchaseManager.isPro && templates.count >= ProFeatureLimits.freeTemplateLimit) {
        ...
        .sheet(item: $proUpgradeReason) { reason in
            ProUpgradeView(reason: reason) {
                proUpgradeReason = nil
            }
        }
    ...
    private func showTemplateCreateView() {
        guard purchaseManager.isPro || templates.count < ProFeatureLimits.freeTemplateLimit else {
            proUpgradeReason = .templateLimit
            return
        }
...
struct TemplateDetailView: View {
            ...
            ScreenContainer {
                ScreenHeader(
                    ...
                    actions: [
                        ...
                        HeaderAction(systemName: "doc.on.doc", tint: NestTaskStyle.ink, label: "複製", isLocked: !purchaseManager.isPro) {
    ...
    private func duplicateTemplate() {
        guard purchaseManager.isPro else {
            proUpgradeReason = .templateDuplication
            return
        }
    ...
    private func showDuplicateConfirmation() {
        guard purchaseManager.isPro else {
            proUpgradeReason = .templateDuplication
            return
        }
...
struct TemplateCreateView: View {
    ...
    @State private var grandchildDraftTitle = ""
    @State private var grandchildDraftResetID = UUID()
    @State private var selectedParentID: UUID?
    @State private var selectedChildParentID: UUID?
    ...
    private var childSteps: [DraftTemplateStep] {
        draftSteps
            .filter { step in
                guard let parentID = step.parentID else { return false }
                return parentSteps.contains { $0.id == parentID }
            }
            .sorted { $0.sortIndex < $1.sortIndex }
    }

    private var selectedChildParent: DraftTemplateStep? {
        guard let selectedChildParentID else { return childSteps.first }
        return childSteps.first { $0.id == selectedChildParentID }
    }
    ...
    var body: some View {
        ScreenContainer {
                ...
                if let selectedParent {
                    ...
                    if !childSteps.isEmpty {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            ForEach(childSteps) { step in
                                if purchaseManager.isPro {
                                    DraftParentSelectionRow(
                                        step: step,
                                        depth: depth(of: step),
                                        isSelected: selectedChildParent?.id == step.id
                                    ) {
                                        selectedChildParentID = step.id
                                    }
                                } else {
                                    LockedDraftParentSelectionRow(
                                        step: step,
                                        depth: depth(of: step)
                                    ) {
                                        proUpgradeReason = .templateDepth
                                    }
                                }
                            }
                        }
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                if let selectedChildParent {
                    Text("追加先：\(selectedChildParent.title)")
                        .font(.system(size: 13, weight: .bold))
                        .foregroundStyle(NestTaskStyle.secondary)

                    HStack(spacing: 10) {
                        TextField("孫タスク名", text: $grandchildDraftTitle)
                            .font(.system(size: 16, weight: .medium))
                            .textFieldStyle(.plain)
                            .id(grandchildDraftResetID)
                            .padding(.horizontal, 14)
                            .frame(height: 48)
                            .background(NestTaskStyle.cardSubtle, in: RoundedRectangle(cornerRadius: 14, style: .continuous))
                        ...
                        Button(action: addGrandchildStep) {
                            Image(systemName: purchaseManager.isPro ? "plus" : "lock.fill")
                                .font(.system(size: 18, weight: .bold))
                                .foregroundStyle(.white)
                                .frame(width: 48, height: 48)
                                .background(NestTaskStyle.teal, in: RoundedRectangle(cornerRadius: 14, style: .continuous))
                        ...
                        .buttonStyle(.plain)
                        .disabled(purchaseManager.isPro && grandchildDraftTitle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty)
                        .opacity(purchaseManager.isPro && grandchildDraftTitle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty ? 0.45 : 1)
                    }
                } else {
                    Text("まず子タスクを追加してください")
                        .font(.system(size: 14, weight: .semibold))
                        .foregroundStyle(NestTaskStyle.secondary)
                }
            }
    ...
    private func addChildStep() {
        ...
        guard !stepTitle.isEmpty, let selectedParent else { return }
        guard purchaseManager.isPro || depth(of: selectedParent) < ProFeatureLimits.freeMaximumDepth else {
            proUpgradeReason = .templateDepth
            return
        }
    ...
    private func addGrandchildStep() {
        guard purchaseManager.isPro else {
            proUpgradeReason = .templateDepth
            return
        }

        let stepTitle = grandchildDraftTitle.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !stepTitle.isEmpty, let selectedChildParent else { return }

        draftSteps.append(
            DraftTemplateStep(
                title: stepTitle,
                parentID: selectedChildParent.id,
                sortIndex: draftSteps.count
            )
        )
        clearGrandchildDraftInput()
    }

    private func clearGrandchildDraftInput() {
        grandchildDraftTitle = ""
        grandchildDraftResetID = UUID()
    }

    private func deleteStep(_ step: DraftTemplateStep) {
        var deletingIDs = DraftTemplateHierarchySupport.descendantIDs(of: step.id, in: draftSteps)
        deletingIDs.insert(step.id)
        draftSteps.removeAll { deletingIDs.contains($0.id) }

        if let selectedParentID, deletingIDs.contains(selectedParentID) {
            self.selectedParentID = parentSteps.first?.id
        }
        if let selectedChildParentID, deletingIDs.contains(selectedChildParentID) {
            self.selectedChildParentID = childSteps.first?.id
        }
    ...
    private func saveTemplate() {
        ...
        guard purchaseManager.isPro || !containsProOnlyDepth(in: draftSteps) else {
            proUpgradeReason = .templateDepth
            return
        }
        guard purchaseManager.isPro || templates.count < ProFeatureLimits.freeTemplateLimit else {
    ...
    private func normalizeDraftSortIndexes() {
        let orderedSteps = DraftTemplateHierarchySupport.flattenedSteps(from: draftSteps)
        draftSteps = orderedSteps
            .enumerated()
            .map { index, step in
                DraftTemplateStep(
                    id: step.id,
    ...
    private func depth(of step: DraftTemplateStep) -> Int {
        DraftTemplateHierarchySupport.depth(of: step.id, in: draftSteps)
    }

    private func containsProOnlyDepth(in steps: [DraftTemplateStep]) -> Bool {
        steps.contains { step in
            DraftTemplateHierarchySupport.depth(of: step.id, in: steps) > ProFeatureLimits.freeMaximumDepth
        }
    }
...
struct TemplateEditView: View {
    ...
    init(template: TaskTemplate) {
        ...
        _selectedChildParentID = State(initialValue: template.sortedSteps.first(where: { $0.parentID != nil })?.id)
    }
    ...
    private func moveDraftStep(draggedID: UUID, targetID: UUID) {
        guard draggedID != targetID else { return }
        guard
            let draggedStep = draftSteps.first(where: { $0.id == draggedID }),
            let targetStep = draftSteps.first(where: { $0.id == targetID }),
            draggedStep.parentID == targetStep.parentID
        else {
            return
        }

        var siblingSteps = draftSteps
            .filter { $0.parentID == draggedStep.parentID }
            .sorted { $0.sortIndex < $1.sortIndex }
        guard
            let sourceIndex = siblingSteps.firstIndex(where: { $0.id == draggedID }),
            let destinationIndex = siblingSteps.firstIndex(where: { $0.id == targetID })
        else {
            return
        }

        let movedStep = siblingSteps.remove(at: sourceIndex)
        siblingSteps.insert(movedStep, at: destinationIndex)
        let siblingSortIndexes = Dictionary(uniqueKeysWithValues: siblingSteps.enumerated().map { index, step in
            (step.id, index)
        })

        withAnimation(.spring(response: 0.28, dampingFraction: 0.86)) {
            for index in draftSteps.indices {
                guard let sortIndex = siblingSortIndexes[draftSteps[index].id] else { continue }
                draftSteps[index].sortIndex = sortIndex
            }
            normalizeDraftSortIndexes()
        }
    }
        ...
        Button(action: action) {
            HStack(spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    if depth > 0 {
                        Text(TaskHierarchyLabels.name(forDepth: depth))
...
private struct LockedDraftParentSelectionRow: View {
    let step: DraftTemplateStep
    let depth: Int
    let action: () -> Void

    private var depthLabel: String {
        "\(TaskHierarchyLabels.nameForAddingUnder(parentDepth: depth))の追加先"
    }
    ...
    var body: some View {
        Button(action: action) {
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                        ...
                        Text("Pro")
                            ...
                            .frame(height: 20)
                            .background(NestTaskStyle.tealSoft, in: Capsule())
                    }

                    Text("\(depthLabel)にできます")
                        .font(.system(size: 11, weight: .semibold))
                        .foregroundStyle(NestTaskStyle.secondary)
                }
                .frame(maxWidth: .infinity, alignment: .leading)
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 14, style: .continuous)
                    .stroke(NestTaskStyle.teal.opacity(0.26), lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    }
...
private struct ProLockBadge: View {
    var body: some View {
        Image(systemName: "lock.fill")
            ...
            .background(NestTaskStyle.teal, in: Circle())
            .overlay(
                Circle()
                    .stroke(NestTaskStyle.card, lineWidth: 2)
            )
    }
}
...
private struct LockedProCard<Content: View>: View {
...
private struct EditableDraftTemplateCard: View {
    ...
    var onMove: ((UUID, UUID) -> Void)? = nil
    @State private var draggingStepID: UUID?

    private var nodes: [DraftTemplateStepNode] {
        DraftTemplateHierarchySupport.nodes(from: steps)
    }
...
private struct EditableDraftStepNodeView: View {
    let node: DraftTemplateStepNode
    let titleBinding: (DraftTemplateStep) -> Binding<String>
    let onSelectParent: (DraftTemplateStep) -> Void
    let onDelete: (DraftTemplateStep) -> Void
    ...
    var body: some View {
        VStack(spacing: 0) {
            EditableDraftStepRow(
                stepID: node.id,
                title: titleBinding(node.step),
                depth: node.depth,
                placeholder: "\(TaskHierarchyLabels.name(forDepth: node.depth))名",
                onFocusParent: { onSelectParent(node.step) },
                onDelete: { onDelete(node.step) },
                draggingStepID: $draggingStepID,
                onMove: onMove
            )
        ...
        Group {
            if let onMove {
                rowContent
                    .onDrop(
                        of: [UTType.text],
                        delegate: DraftStepDropDelegate(
                            targetStepID: stepID,
                            draggingStepID: $draggingStepID,
                            onMove: onMove
                        )
...
private struct DraftStepDropDelegate: DropDelegate {
    let targetStepID: UUID
    @Binding var draggingStepID: UUID?
    let onMove: (UUID, UUID) -> Void

    func dropEntered(info: DropInfo) {
        guard let draggingStepID, draggingStepID != targetStepID else { return }
        onMove(draggingStepID, targetStepID)
    }
...
struct ExecutionTaskDetailView: View {
    ...
    var body: some View {
        ...
        .sheet(isPresented: $isShowingNameEditor) {
            ExecutionTaskNameEditView(task: task) { taskTitle, templateTitle, reflectsToTemplate in
                saveNames(
                    taskTitle: taskTitle,
                    templateTitle: templateTitle,
                    reflectsToTemplate: reflectsToTemplate
                )
            }
        }
        .sheet(item: $proUpgradeReason) { reason in
    ...
    private func showNameEditor() {
        guard purchaseManager.isPro else {
            proUpgradeReason = .executionTaskEditing
            return
        }
    ...
    private func saveNames(taskTitle: String, templateTitle: String?, reflectsToTemplate: Bool) {
        guard purchaseManager.isPro else {
            proUpgradeReason = .executionTaskEditing
            return
        }
        ...
        let trimmedTemplateTitle = templateTitle?.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !trimmedTaskTitle.isEmpty else { return }
        if templateTitle != nil, trimmedTemplateTitle?.isEmpty != false {
            return
        }

        task.title = trimmedTaskTitle

        if let trimmedTemplateTitle {
            task.templateTitle = trimmedTemplateTitle
            if reflectsToTemplate, let template = task.template {
                template.title = trimmedTemplateTitle
            }
        }
        ...
        do {
            ...
            modelContext.rollback()
            persistenceMessage = BackupMessage(title: "名前を保存できませんでした", message: error.localizedDescription)
        }
    }

    private func addNewStep(scope: ExecutionStepAdditionScope) {
        if let sanitizedSelectedParentID,
           !purchaseManager.isPro,
           ExecutionStepHierarchySupport.depth(of: sanitizedSelectedParentID, in: task.steps) >= ProFeatureLimits.freeMaximumDepth {
            proUpgradeReason = .templateDepth
            return
        }
...
private struct ExecutionTaskNameEditView: View {
    @Environment(\.dismiss) private var dismiss

    let canEditTemplateName: Bool
    let canReflectToTemplate: Bool
    let onSave: (String, String?, Bool) -> Void

    @State private var taskTitle: String
    @State private var templateTitle: String
    @State private var reflectsToTemplate = false

    init(task: ExecutionTask, onSave: @escaping (String, String?, Bool) -> Void) {
        let editableTemplateTitle = task.templateTitle ?? task.template?.title ?? ""
        self.canEditTemplateName = !editableTemplateTitle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
        self.canReflectToTemplate = task.template != nil
        self.onSave = onSave
        _taskTitle = State(initialValue: task.title)
        _templateTitle = State(initialValue: editableTemplateTitle)
    }
    ...
    private var canSave: Bool {
        !trimmedTaskTitle.isEmpty
            && (!canEditTemplateName || !trimmedTemplateTitle.isEmpty)
    }
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    TemplateFormTextField(
                        title: "実行タスク名",
                        placeholder: "今回のタスク名",
                        text: $taskTitle
                    )

                    if canEditTemplateName {
                        TemplateFormTextField(
                            title: "元テンプレート名",
                            placeholder: "テンプレート名",
                            text: $templateTitle
                        ...
                        Toggle(isOn: reflectBinding) {
                            VStack(alignment: .leading, spacing: 4) {
                                Text("テンプレートにも反映")
                                    .font(.system(size: 15, weight: .bold))
                                    .foregroundStyle(NestTaskStyle.ink)

                                Text(canReflectToTemplate ? "元テンプレートの名前も更新します" : "元テンプレートがないため今回のタスク内だけに保存します")
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .topBarTrailing) {
                    Button("保存") {
                        onSave(
                            trimmedTaskTitle,
                            canEditTemplateName ? trimmedTemplateTitle : nil,
                            reflectsToTemplate && canReflectToTemplate
                        )
                    }
                    .font(.system(size: 15, weight: .bold))
                    .disabled(!canSave)
    ...
    private var reflectBinding: Binding<Bool> {
        Binding {
            reflectsToTemplate && canReflectToTemplate
        } set: { newValue in
            reflectsToTemplate = canReflectToTemplate && newValue
        }
    }
}
...
private struct ExecutionStepAddCard: View {
    ...
    private var selectedParentIsLocked: Bool {
        guard let selectedParentID else { return false }
        return !purchaseManager.isPro
            && ExecutionStepHierarchySupport.depth(of: selectedParentID, in: task.steps) >= ProFeatureLimits.freeMaximumDepth
    }
                ...
                Menu {
                    ...
                    ForEach(task.sortedSteps) { step in
                        Button {
                            ...
                            if !purchaseManager.isPro
                                && ExecutionStepHierarchySupport.depth(of: step.id, in: task.steps) >= ProFeatureLimits.freeMaximumDepth {
                                Label(step.title, systemImage: "lock.fill")
                            } else {
                                Text(step.title)
                            }
                        }
...
struct HistoryView: View {
    ...
    @State private var searchQuery = ""
    @State private var selectedCategory = TemplateCategorySupport.allCategory
    @State private var proUpgradeReason: ProUpgradeReason?
    ...
    private var historyCategories: [String] {
        let categories = Set(completedTasks.map(categoryLabel(for:)))
            .subtracting([TemplateCategorySupport.allCategory])
            .sorted { $0.localizedStandardCompare($1) == .orderedAscending }
        return [TemplateCategorySupport.allCategory] + categories
    }

    private var filteredCompletedTasks: [ExecutionTask] {
        guard purchaseManager.isPro else {
            return Array(completedTasks.prefix(ProFeatureLimits.freeHistoryLimit))
        }

        let trimmedQuery = searchQuery.trimmingCharacters(in: .whitespacesAndNewlines)
        return completedTasks.filter { task in
            let matchesCategory = selectedCategory == TemplateCategorySupport.allCategory
                || categoryLabel(for: task) == selectedCategory
            let matchesQuery = trimmedQuery.isEmpty || [
                task.title,
                task.templateNameLabel,
                task.cadenceLabel,
                categoryLabel(for: task),
                task.steps.map(\.title).joined(separator: " ")
            ]
            .joined(separator: " ")
            .localizedCaseInsensitiveContains(trimmedQuery)
    ...
    var body: some View {
        ScreenContainer {
            ScreenHeader(
                ...
                subtitle: "完了した実行タスク",
                actions: [
                    HeaderAction(systemName: "magnifyingglass", tint: NestTaskStyle.ink, label: "履歴検索", isLocked: !purchaseManager.isPro) {
                        toggleHistorySearch()
                    },
                    HeaderAction(systemName: "line.3.horizontal.decrease.circle", tint: NestTaskStyle.ink, label: "履歴フィルタ", isLocked: !purchaseManager.isPro) {
                        toggleHistoryFilter()
                    },
            ...
            if purchaseManager.isPro && isShowingSearch {
                HistorySearchField(query: $searchQuery)
            }

            if purchaseManager.isPro && isShowingFilter {
                CategorySelector(categories: historyCategories, selection: $selectedCategory)
            }

            if !purchaseManager.isPro {
                ProLockedNoticeCard(
                    title: "無料版では直近\(ProFeatureLimits.freeHistoryLimit)件まで表示します",
                    message: "履歴検索・フィルタ・詳細確認、すべての履歴閲覧はNestTask Proで使えます。"
                )
            }
        ...
        .onChange(of: historyCategories) { _, newCategories in
            guard !newCategories.contains(selectedCategory) else { return }
            selectedCategory = TemplateCategorySupport.allCategory
        }
    }
    ...
    private func toggleHistoryFilter() {
        guard purchaseManager.isPro else {
            proUpgradeReason = .history
            return
        }
    ...
    private func categoryLabel(for task: ExecutionTask) -> String {
        let category = task.category?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        return category.isEmpty ? "カテゴリなし" : category
    }
}
...
private struct AppInfoView: View {
    ...
    @State private var proUpgradePresentation: AppInfoProUpgradePresentation?
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 18) {
                    ...
                    SettingsSection(title: "NestTask Pro") {
                        VStack(spacing: 0) {
                            ProStatusSettingsRow(isPro: purchaseManager.isPro)
                            ...
                            if !purchaseManager.isPro {
                                ...
                                SettingsActionRow(
                                    title: "Proにアップグレード",
                                    subtitle: "上限解除、履歴検索、JSONバックアップなど",
                                    systemName: "sparkles",
                                    tint: NestTaskStyle.teal
                                ) {
                                    proUpgradePresentation = .general
                                }
                            }
                            ...
                            SettingsActionRow(
                                title: "購入を復元",
                                subtitle: purchaseManager.isRestoringPurchases ? "復元中" : "機種変更後もApple IDから復元できます",
                        ...
                        VStack(spacing: 0) {
                            ForEach(AppearanceMode.allCases) { mode in
                                AppearanceModeRow(
                                    ...
                                    isLocked: mode != .system && !purchaseManager.isPro
                                ) {
        ...
        .presentationDetents([.medium, .large])
        .onAppear(perform: enforceFreeAppearanceMode)
        .onChange(of: purchaseManager.isPro) { _, _ in
            enforceFreeAppearanceMode()
        }
    ...
    private func selectAppearanceMode(_ mode: AppearanceMode) {
        guard mode == .system || purchaseManager.isPro else {
            proUpgradePresentation = .reason(.appearance)
            return
        }
    ...
    private func enforceFreeAppearanceMode() {
        guard !purchaseManager.isPro, appearanceModeRawValue != AppearanceMode.system.rawValue else { return }
...
private struct ProStatusSettingsRow: View {
    let isPro: Bool

    var body: some View {
        HStack(spacing: 12) {
            Image(systemName: isPro ? "checkmark.seal.fill" : "lock.open")
                .font(.system(size: 17, weight: .bold))
                .foregroundStyle(isPro ? NestTaskStyle.green : NestTaskStyle.secondary)
                .frame(width: 30, height: 30)
                .background(
                    isPro ? NestTaskStyle.green.opacity(0.12) : NestTaskStyle.cardSubtle,
                    in: Circle()
                )
...
private struct HeaderIconButton: View {
    ...
    var body: some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel(action.isLocked ? "\(action.label)、Pro機能" : action.label)
    }
}
    ...
    private func toggleCompletion() {
        withAnimation(.easeInOut(duration: 0.18)) {
            if let task = step.executionTask {
                ProgressService.toggleCompletion(for: step, in: task, descendantSteps: descendantSteps)
            }
        }
...
private struct ProLockedNoticeCard: View {
    let title: String
    let message: String
